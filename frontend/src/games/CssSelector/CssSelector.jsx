...
export default function CssSelector() {
  ...
  useEffect(() => {
    const fetchCount = async () => {
      try {
        const res = await questionsAPI.getCount('cssselector');
  ...
  const startGame = async () => {
    ...
    try {
      const res = await questionsAPI.getQuestions('cssselector', 20);
  ...
  const handleAnswer = async (selected) => {
    ...
    if (user && currentChallenge._id) {
      try {
        await answersAPI.submit('cssselector', {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      >
        <h1 className="text-4xl font-bold mb-2">🥷 CSS Selector Ninja</h1>
        <p className="text-gray-400">Choose the correct CSS selector</p>
      </motion.div>
      ...
      <div className="card space-y-6">
        {gameState === 'idle' && (
          ...
          >
            ...
            <h2 className="text-2xl font-semibold">Ready to master CSS selectors?</h2>
            <p className="text-gray-400 max-w-md mx-auto">
              Test your CSS selector knowledge! Choose the right selector.
            </p>
            {questionCount > 0 && (
